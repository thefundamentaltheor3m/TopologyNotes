...
\end{boxdefinition}
\begin{boxtheorem}\label{Ch3:Thm:Embedding_Prod_of_Sep_Points_Cl}
    If $f_i:X\rightarrow Y_i$ with $i\in I$ are such that each $f_i$ is continuous and separates points from closed sets, then $F : X \to \prod_{i}Y_i$ given by
    \begin{align*}
        F(x)=(f_i(x))_{i\in I}
    \end{align*}
    is an embedding of $X$ into $\prod_{i}Y_i$.
\end{boxtheorem}
...
\begin{boxlnotation}
    Denote by $\Delta$ the set of all dyadic rationals contained in $(0, 1)$.
...
\begin{proof}[Proof of Urysohn's Lemma (\Cref{SP:Urysohn}).]
    We will proceed by constructing sets $\setst{U_q}{q \in \Delta}$ such that
    \begin{enumerate}[noitemsep]
        \item $C \ssq U_q \ssq X \setminus D$ for all $q \in \Delta$.
        \item If $p, q \in \Delta$ and $p < q$ then $\cl{U_p} \ssq U_q$.
    \end{enumerate}
    To do this, we will actually build $\setst{U_q, V_q}{q \in \Delta}$ such that for all $q \in \Delta$,
    \begin{enumerate}[noitemsep]
        \item $U_q$ is open
        \item $V_q$ is closed
    ...
    %         % U 1/4, V 1/4
    %         \draw [blue, thin, domain=-30:30, samples=40] plot (0.2 * {\cos{\t}}, 0.2 * {\sin{\t}});
    %         \draw [blue, thin, domain=-30:30, samples=40] plot (0.3 * {\cos{\x}}, 0.3 * {\sin{\x}});
            
    %         % U 1/2, V 1/2
    %         \draw [blue,thin,domain=-30:30, samples=100] plot ({0.4 * cos(\x)}, {0.4 * sin(\x)});
    %         \draw [blue,thin,domain=-30:30, samples=100] plot ({0.6 * cos(\x)}, {0.6 * sin(\x)});
    %     \end{tikzpicture}
    %     \caption{Caption}
    %     \label{fig:placeholder}
    % \end{figure}
    ...
    %por supuesto
    We will now get on to the construction of our continuous function. To do so, we define $f:X \to [0,1]$ such that, if there us no $r\in \Delta$ with $x\in U_r$, then we let $f(x) := 1$; otherwise, we define $f(x) := \inf\!\setst{r\in \Delta}{x\in U_r}$.

    It is ``easy'' to see that if $x \in D$, then $f(x)=1$ and that if $x\in C$, then $ f(x)=\inf(\Delta)=0$. However we must now get on to the trickier part: showing that $f$ is continuous.
    
    We begin by claiming that for all $x \in X$ and $p \in \Delta$,
    \begin{enumerate}[label = (\alph*), noitemsep]
        \item If $x \in \cl\of{U_p}$, $f(x) \leq p$
        \item If $x \notin \cl\of{U_p}$, $f(x) \geq p$
    \end{enumerate}
    Indeed, if $x \in \cl\of{U_p}$, then $x \in U_q$ for all $q \in \Delta$ with $q > p$, so $f(x) \leq p$. On the other hand, if $x \notin \cl\of{U_p}$, then assume, for contradiction, that $f(x) < p$. Then we can easily see that there must be some $r < p$ and $x \in U_r$ such that $f(x) \leq r < p$. But we know that $r<p$ and $x\in U_r\implies x\in U_p\ssq \cl\of{U_p}$ - a contradiction! So indeed, things are just as we wanted.

    Now that we know our claims are true, taking the contrapositive of each claim we have that for all $x \in X$ and $p \in \Delta$,
    \begin{enumerate}[label = (\alph*), noitemsep]
        \item If $f(x) > p$ then $x \notin \cl\of{U_p}$ (and thus $x \notin U_p$, so $f(x) \geq p$).
    ...
    We begin by showing that $f\inv\!\brac{(a, b)}$ is open. We do this by showing every point has an open neighbourhood. Fix $x \in f\inv\!\brac{(a, b)}$. We know that $a < f(x) < b$. Find $p, q \in \Delta$ such that $a < p < f(x) < q < b$. Then $f(x) > p$, so $x \notin \cl\of{U_p}$. Similarly, $f(x) < q$, so $x \in U_q$. Now, let $V = U_q \setminus \cl{U_p}$. Observe that $x \in V$. Moreover, for all $y \in V$, $y \in U_q$, so $f(y) \leq q$, and $y \notin U_p$, so $f(y) \geq p$. Then, we can see that $f[V] \ssq [p, q] \ssq (a, b)$.
    ...
    So for $f\inv [[0,a)]$ we let $f(x)<a$ and find $p\in \Delta$ with $f(x)<p<a$, $x\in U_p$ and $f[U_p]\ssq [0,p]\ssq [0,a)$. Meanwhile, for $f\inv [(b,1]]$ we let $f(x)>b$ we find $q\in \Delta$ such that $b<q<f(x)$ and the $x\notin \cl{U_q}$ and $y\in X\setminus \cl{U_q}$ then $y\notin U_q\implies f(y)\geq q>b$. 
\end{proof}
...
\begin{boxlemma}\label{Ch3:Lemma:Lindelof_T3_is_T4}
...
\begin{proof}[Proof of \Cref{SP:Urysohn_Metrisation}]
    Let $X$ be a 2nd countable $T_3$ space. To embed $X$ into $[0, 1]^{\N}$, it will suffice to find a countable set $\F$ of continuous functions from $X$ to $[0, 1]$ that separate points from closed sets.

    \textbf{Why?} Since $\F$ is countable, we can enumerate such a set $\F$ as functions $\set{f_1, f_2, \ldots}$ and define $F : X \to [0, 1]^{\N} : x \mapsto \parenth{f_n(x)}_{n \in \N}$. Then, if $\F$ separates points from closed sets, \Cref{Ch3:Thm:Embedding_Prod_of_Sep_Points_Cl} immediately tells us that $F$ is an embedding!

    So now let's get to work and actually find such a family!

    We will make use of the fact that $X$ is second countable. This means precisely that the topology of $X$ has a countable basis $B$. For all pairs $\parenth{U, V}$ with $U, V \in B$ and $\cl\of{U} \ssq V$, we observe that $\cl\of{U} \cap \parenth{X \setminus V} = \emptyset$. By \Cref{Ch3:Lemma:2nd_countable_is_Lindelof}, since $X$ is 2nd countable, $X$ must be Lindelöf; thus, by \Cref{Ch3:Lemma:Lindelof_T3_is_T4}, since $X$ is also $T_3$, $X$ is $T_4$. In particular, $X$ is normal. Thus, since $\cl\of{U}$ and $X \setminus V$ are disjoint, closed subsets of $X$, we can apply Urysohn's Lemma (\Cref{SP:Urysohn}) to define a continuous function $f_{U, V} : X \to [0, 1]$ that maps $\cl\of{U}$ to $\set{0}$ and $X \setminus V$ to $\set{1}$. We then define $\F := \setst{f_{U, V}}{U, V \in B}$. Since $B$ is countable, there are countably many pairs $(U, V)$, and thus countably many $f_{U, V}$ in $\F$.

    It just remains to show that $\F$ separates points from closed sets. Let $F \ssq X$ be closed. Fix some $x \in X \setminus F$. Clearly $X \setminus F$ is open, so there is some $V \in B$ such that $x \in V$ and $V \ssq X \setminus F$ (meaning $V \cap F = \emptyset$). Since $X$ is $T_3$, $X$ is regular; thus, we know that there is an open set $U'$ that is a neighbourhood of $x$ whose closure is contained in $V$. Ie, we have $x \in U'$ and $\cl\of{U'} \ssq V$. Indeed, we can find a \textit{basic} $U \in B$ such that $x \in U \ssq U'$ and $\cl\of{U} \ssq \cl\of{U'} \ssq V$.
