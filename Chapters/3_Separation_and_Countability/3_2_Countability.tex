...
Suppose, as before, that $X$ is a $T_{3.6}$ topological space. Define
\begin{align*}
...
\begin{proof}
    ...
    For all $a \in A$, we know that $a \notin B$, so $a \in X \setminus 
    B$, which is an open set. Then there is some open $U_a \ni a$ such that $U \cap B = \emptyset$. This gives us an open cover of $A$, which we know admits a countable subcover $\parenth{U_{a_n}}_{n \in \N}$. In similar fashion, we can construct an open cover $\parenth{V_{b_n}}_{n \in \N}$.
    ...
    Now, define
    \begin{align*}
        Y_n &:= U_n \cap \parenth{\bigcap_{k=1}^{n} \parenth{X \setminus \cl\of{V_{b_n}}}} & Y &:= \bigcup_{n \in \N} Y_n \\
        Z_n &:= V_n \cap \parenth{\bigcap_{k=1}^{n} \parenth{X \setminus \cl\of{U_{a_n}}}} & Z &:= \bigcup_{n \in \N} Z_n
    \end{align*}
    The point is that for each $n \in \N$, since $A \cap \cl\of{V_{b_n}}$, we know $A \ssq X \setminus \cl\of{V_{b_n}}$. Similarly,for each $n \in \N$, since $B \cap \cl\of{U_{a_n}}$, we know $B \ssq X \setminus \cl\of{U_{a_n}}$.  Thus, 
    \begin{align*}
        A \ssq \bigcap_{n \in \N} \parenth{X \setminus \cl\of{V_{b_n}}}
        &&
        B \ssq \bigcap_{n \in \N} \parenth{X \setminus \cl\of{U_{a_n}}}
    \end{align*}
    Moreover, it turns out that for all $m, n \in \Z$, $Y_m \cap Z_n = \emptyset$. Indeed, suppose this is not true. Then there are some $m, n \in \N$ (with $m \leq n$ WLoG) such that there is some $x \in Y_m \cap Z_n$. Since $x \in Y_m$, $x \in U_m$, and since $x \in Z_n$ and $m \leq n$, $x \in X \setminus \cl\of{U_m}$. But this is impossible. Thus, for all $m, n \in \Z$, $Y_m \cap Z_n = \emptyset$.

    This implies that $Y \cap Z = \emptyset$. So $X$ is $T_4$.
\end{proof}

We are now ready to prove Urysohn's Metrisation Theorem.

\begin{proof}[Proof of \Cref{SP:Urysohn_Metrisation}]
    Let $X$ be a 2nd countable $T_3$ space. We need to show that $X$ is metrisable.
    
    By \Cref{Ch3:Lemma:2nd_countable_is_Lindelof}, if $X$ is 2nd countable, then $X$ is Lindelöf. To embed $X$ into $[0, 1]^{\N}$, it will suffice to find a countable set $\F$ of continuous functions from $X$ to $[0, 1]$ that separate points from closed sets.

    \textbf{Why?} Well, first enumerate such a set $\F$ as functions $\set{f_1, f_2, \ldots}$ and define $F : X \to [0, 1]^{\N} : x \mapsto \parenth{f_n(x)}_{n \in \N}$. As $\F$ separates points from closed sets, \sorry. Note that it is crucial that $\F$ is countable for this\todo{What?} to work.
